function [exp_mat,Gamma]=create_mapping_Gamma_no_rep(grids,map_volumes,nodes,nodes_red,externals_grids)
...
mapping_surf_12_se=zeros((Nx)*(Ny+1)*(Nz),1);
mapping_surf_34_se=zeros((Nx+1)*(Ny)*(Nz),1);
mapping_surf_56_se=zeros((Nx)*(Ny)*(Nz+1),1);

mapping_surf_1234=zeros((2*(Nx+1)-1)*(2*(Ny+1)-1)*(Nz),1);
mapping_surf_1256=zeros((Nx)*(2*(Ny+1)-1)*(2*(Nz+1)-1),1);
mapping_surf_3456=zeros((2*(Nx+1)-1)*(Ny)*(2*(Nz+1)-1),1);
...
ind_r=zeros(nnz_surf_max,1);
ind_c=zeros(nnz_surf_max,1);
        ...
        for cont3=1:Nz
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont,cont2-1,cont3)) &&...
                        (~(externals_grids{k,2}(cont,cont2,cont3)) || check_om)
    ...
    for cont2=1:Ny-1
        for cont3=1:Nz
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont,cont2+1,cont3)) &&...
                        (~(externals_grids{k,1}(cont,cont2,cont3)) || check_om)
        ...
        for cont3=1:Nz
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont-1,cont2,cont3)) &&...
                        (~(externals_grids{k,4}(cont,cont2,cont3)) || check_om)
    ...
    for cont2=1:Ny
        for cont3=1:Nz
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont+1,cont2,cont3)) &&...
                        (~(externals_grids{k,3}(cont,cont2,cont3)) || check_om)
        ...
        for cont3=2:Nz
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont,cont2,cont3-1)) &&...
                        (~(externals_grids{k,6}(cont,cont2,cont3)) || check_om)
    ...
    for cont2=1:Ny
        for cont3=1:Nz-1
            for k=1:num_grids
                ...
                if(grids{k}(cont,cont2,cont3)) && ~(grids{k}(cont,cont2,cont3+1)) &&...
                        (~(externals_grids{k,5}(cont,cont2,cont3)) || check_om)
...
Gamma = sparse(ind_r(1:contat_tot),ind_c(1:contat_tot),ones(contat_tot,1),length(nodes_red),nnz_surf);

exp_mat.N1=0;
exp_mat.N2=0;
exp_mat.N3=0;
exp_mat.exp_P=cell(3,3);

inn=find(mapping_surf_12_se~=0);
N1=length(inn);
exp_mat.N1=N1;
exp_mat.exp_P{1,1}=sparse(inn,mapping_surf_12_se(inn),ones(N1,1),size(mapping_surf_12_se,1),nnz_surf);

inn=find(mapping_surf_34_se~=0);
N2=length(inn);
exp_mat.N2=N2;
exp_mat.exp_P{2,2}=sparse(inn,mapping_surf_34_se(inn),ones(N2,1),size(mapping_surf_34_se,1),nnz_surf);

inn=find(mapping_surf_56_se~=0);
N3=length(inn);
exp_mat.N3=N3;
exp_mat.exp_P{3,3}=sparse(inn,mapping_surf_56_se(inn),ones(N3,1),size(mapping_surf_56_se,1),nnz_surf);


inn1=find(mapping_surf_1234<=num_ele_12);
inn2=find(mapping_surf_1234~=0);
inn=intersect(inn1,inn2);
N4=length(inn);
exp_mat.exp_P{2,1}=sparse(inn,mapping_surf_1234(inn),ones(N4,1),size(mapping_surf_1234,1),nnz_surf);

inn=find(mapping_surf_1234>num_ele_12);
N5=length(inn);
exp_mat.exp_P{1,2}=sparse(inn,mapping_surf_1234(inn),ones(N5,1),size(mapping_surf_1234,1),nnz_surf);


inn1=find(mapping_surf_1256<=num_ele_12);
inn2=find(mapping_surf_1256~=0);
inn=intersect(inn1,inn2);
N6=length(inn);
exp_mat.exp_P{3,1}=sparse(inn,mapping_surf_1256(inn),ones(N6,1),size(mapping_surf_1256,1),nnz_surf);

inn=find(mapping_surf_1256>num_ele_12+num_ele_34);
N7=length(inn);
exp_mat.exp_P{1,3}=sparse(inn,mapping_surf_1256(inn),ones(N7,1),size(mapping_surf_1256,1),nnz_surf);



inn1=find(mapping_surf_3456<=num_ele_12+num_ele_34);
inn2=find(mapping_surf_3456~=0);
inn=intersect(inn1,inn2);
N8=length(inn);
exp_mat.exp_P{3,2}=sparse(inn,mapping_surf_3456(inn),ones(N8,1),size(mapping_surf_3456,1),nnz_surf);

inn=find(mapping_surf_3456>num_ele_12+num_ele_34);
N9=length(inn);
exp_mat.exp_P{2,3}=sparse(inn,mapping_surf_3456(inn),ones(N9,1),size(mapping_surf_3456,1),nnz_surf);
