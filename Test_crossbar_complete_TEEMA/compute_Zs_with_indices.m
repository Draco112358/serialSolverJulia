function [Zs_info,ind_cond_rug_x,ind_cond_rug_y,ind_cond_rug_z]=compute_Zs_with_indices(materials,li_mats,maps_Zs,sx,sy,sz)

mx=size(li_mats.lix_mat,1);
my=size(li_mats.liy_mat,1);
mz=size(li_mats.liz_mat,1);

contenitore=zeros(length(materials),6)+1j*zeros(length(materials),6);
...
for cont=1:length(materials)
    ...
    if sigmar~=0
        ...
        contenitore(cont,1)=0.5*sx/sy*(1+1j)*sqrt(2*pi*1e-7/sigmar);
        contenitore(cont,2)=0.5*sx/sz*(1+1j)*sqrt(2*pi*1e-7/sigmar);
        contenitore(cont,3)=0.5*sy/sx*(1+1j)*sqrt(2*pi*1e-7/sigmar);
        contenitore(cont,4)=0.5*sy/sz*(1+1j)*sqrt(2*pi*1e-7/sigmar);
        contenitore(cont,5)=0.5*sz/sx*(1+1j)*sqrt(2*pi*1e-7/sigmar);
        contenitore(cont,6)=0.5*sz/sy*(1+1j)*sqrt(2*pi*1e-7/sigmar);
...
Z_surf_x=cell(4,Ntot);
Z_surf_y=cell(4,Ntot);
Z_surf_z=cell(4,Ntot);

Z_surf_rug_x=cell(4,Ntot);
Z_surf_rug_y=cell(4,Ntot);
Z_surf_rug_z=cell(4,Ntot);

ind_cond_x=cell(4,Ntot);
ind_cond_y=cell(4,Ntot);
ind_cond_z=cell(4,Ntot);

ind_cond_rug_x=cell(4,Ntot);
ind_cond_rug_y=cell(4,Ntot);
ind_cond_rug_z=cell(4,Ntot);
...
Zswx=zeros(mx,4)+1j*zeros(mx,4);
Zswy=zeros(my,4)+1j*zeros(my,4);
Zswz=zeros(mz,4)+1j*zeros(mz,4);

ind_x_xy_1=find(maps_Zs.x_xy(:,1)>0);
ind_x_xy_2=find(maps_Zs.x_xy(:,2)>0);

ind_x_zx_1=find(maps_Zs.x_zx(:,1)>0);
ind_x_zx_2=find(maps_Zs.x_zx(:,2)>0);

ind_y_xy_1=find(maps_Zs.y_xy(:,1)>0);
ind_y_xy_2=find(maps_Zs.y_xy(:,2)>0);

ind_y_yz_1=find(maps_Zs.y_yz(:,1)>0);
ind_y_yz_2=find(maps_Zs.y_yz(:,2)>0);

ind_z_zx_1=find(maps_Zs.z_zx(:,1)>0);
ind_z_zx_2=find(maps_Zs.z_zx(:,2)>0);

ind_z_yz_1=find(maps_Zs.z_yz(:,1)>0);
ind_z_yz_2=find(maps_Zs.z_yz(:,2)>0);
...
for cont=1:length(materials)
    ...
    if contenitore(cont,1)~=0
        ...
        ind_m=find(cont==li_mats.lix_mat(:,1));
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_x_xy_cd=find(maps_Zs.x_xy(:,3)==cont2);
                    ind_rg=intersect(ind_coder_1,ind_x_xy_cd);
                    ind_rug_1(ind_rg)=1;
                end
            end
        end
        ind_rug_1=find(ind_rug_1>0);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_x_xy_cd=find(maps_Zs.x_xy(:,4)==cont2);
                    ind_rg=intersect(ind_coder_3,ind_x_xy_cd);
                    ind_rug_3(ind_rg)=1;
                end
            end
        end
        ind_rug_3=find(ind_rug_3>0);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_x_zx_cd=find(maps_Zs.x_zx(:,3)==cont2);
                    ind_rg=intersect(ind_coder_2,ind_x_zx_cd);
                    ind_rug_2(ind_rg)=1;
                end
            end
        end
        ind_rug_2=find(ind_rug_2>0);
        ...
        ind_m=find(cont==li_mats.lix_border(:,2));
        ...
        ind_m=find(cont==li_mats.liy_mat(:,1));
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_y_xy_cd=find(maps_Zs.y_xy(:,3)==cont2);
                    ind_rg=intersect(ind_coder_1,ind_y_xy_cd);
                    ind_rug(ind_rg)=1;
                end
            end
        end
        ind_rug_1=find(ind_rug>0);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_y_xy_cd=find(maps_Zs.y_xy(:,4)==cont2);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_y_yz_cd=find(maps_Zs.y_yz(:,3)==cont2);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_y_yz_cd=find(maps_Zs.y_yz(:,4)==cont2);
        ...
        ind_m=find(cont==li_mats.liy_border(:,1));
        ind=intersect(ind_coder_1,ind_m);
        ...
        ind_m=find(cont==li_mats.liy_border(:,2));
        ind=intersect(ind_coder_3,ind_m);
        ...
        ind_m=find(cont==li_mats.liz_mat(:,1));
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_z_zx_cd=find(maps_Zs.z_zx(:,3)==cont2);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_z_zx_cd=find(maps_Zs.z_zx(:,4)==cont2);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_z_yz_cd=find(maps_Zs.z_yz(:,3)==cont2);
        ...
        for cont2=1:length(materials)
            if cont~=cont2
                if ~isempty(find(cont2==id_diel, 1))
                    ind_z_yz_cd=find(maps_Zs.z_yz(:,4)==cont2);
        ...
        ind_m=find(cont==li_mats.liz_border(:,1));
        ind=intersect(ind_coder_1,ind_m);
        ...
        ind_m=find(cont==li_mats.liz_border(:,2));
        ind=intersect(ind_coder_3,ind_m);
...
z1_x=zeros(mx,1)+1j*zeros(mx,1);
z2_x=zeros(mx,1)+1j*zeros(mx,1);
z3_x=zeros(mx,1)+1j*zeros(mx,1);
z4_x=zeros(mx,1)+1j*zeros(mx,1);
...
ind1=find(abs(z1_x)~=0);
ind2=find(abs(z2_x)~=0);

ipara=intersect(ind1,ind2);
zsx1=z1_x+z2_x;
zsx1(ipara)=z1_x(ipara).*z2_x(ipara)./(z1_x(ipara)+z2_x(ipara));

ind1=find(abs(z3_x)~=0);
ind2=find(abs(z4_x)~=0);
...
z1_y=zeros(my,1)+1j*zeros(my,1);
z2_y=zeros(my,1)+1j*zeros(my,1);
z3_y=zeros(my,1)+1j*zeros(my,1);
z4_y=zeros(my,1)+1j*zeros(my,1);
...
ind1=find(abs(z1_y)~=0);
ind2=find(abs(z2_y)~=0);

ipara=intersect(ind1,ind2);
zsy1=z1_y+z2_y;
zsy1(ipara)=z1_y(ipara).*z2_y(ipara)./(z1_y(ipara)+z2_y(ipara));

ind1=find(abs(z3_y)~=0);
ind2=find(abs(z4_y)~=0);
...
z1_z=zeros(mz,1)+1j*zeros(mz,1);
z2_z=zeros(mz,1)+1j*zeros(mz,1);
z3_z=zeros(mz,1)+1j*zeros(mz,1);
z4_z=zeros(mz,1)+1j*zeros(mz,1);
...
ind1=find(abs(z1_z)~=0);
ind2=find(abs(z2_z)~=0);

ipara=intersect(ind1,ind2);
zsz1=z1_z+z2_z;
zsz1(ipara)=z1_z(ipara).*z2_z(ipara)./(z1_z(ipara)+z2_z(ipara));

ind1=find(abs(z3_z)~=0);
ind2=find(abs(z4_z)~=0);
...
Zs_info_internal_edges=find(abs(Zs_info_Zs)==0);
...
Zs_info = struct('Zs', complex(Zs_info_Zs), 'internal_edges', Zs_info_internal_edges,...
