...
function mesher_FFT(use_escalings,materials,sx,sy,sz,grids,centri_vox,externals_grids,mapping_vols,ports,lumped_elements)
    ...
    escalings = Dict("Lp" => 1, "P" => 1, "R" => 1, "Cd" => 1, "Is" => 1, "Yle" => 1, "freq" => 1, "time" => 1)
    if use_escalings == 1
        escalings["Lp"] = 1e6
        escalings["P"] = 1e-12
        escalings["R"] = 1e-3
    ...
    ind_Lp = Array{Any}(3)
    expansions["N_ind"] = Array{Any}(3)
    expansions["mat_map_Lp"] = Array{Any}(3, 3)
    ...
    expansions["mat_map_Lp"][1, 1] = sparse(ind_Lp[1][:, 1], ind_Lp[1][:, 2], ones(size(ind_Lp[1], 1), 1), expansions["N_ind"][1], size(ind_Lp[1], 1))
    lati_tenere = li_mats["lix_border"][:, 1] .!= 0
    ind_aux = ind_Lp[1][lati_tenere, :]
    expansions["mat_map_Lp"][1, 2] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][1], size(ind_Lp[1], 1))
    lati_tenere = li_mats["lix_border"][:, 2] .!= 0
    ind_aux = ind_Lp[1][lati_tenere, :]
    expansions["mat_map_Lp"][1, 3] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][1], size(ind_Lp[1], 1))
    ...
    expansions["mat_map_Lp"][2, 1] = sparse(ind_Lp[2][:, 1], ind_Lp[2][:, 2], ones(size(ind_Lp[2], 1), 1), expansions["N_ind"][2], size(ind_Lp[2], 1))
    lati_tenere = li_mats["liy_border"][:, 1] .!= 0
    ind_aux = ind_Lp[2][lati_tenere, :]
    expansions["mat_map_Lp"][2, 2] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][2], size(ind_Lp[2], 1))
    lati_tenere = li_mats["liy_border"][:, 2] .!= 0
    ind_aux = ind_Lp[2][lati_tenere, :]
    expansions["mat_map_Lp"][2, 3] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][2], size(ind_Lp[2], 1))
    ...
    expansions["mat_map_Lp"][3, 1] = sparse(ind_Lp[3][:, 1], ind_Lp[3][:, 2], ones(size(ind_Lp[3], 1), 1), expansions["N_ind"][3], size(ind_Lp[3], 1))
    lati_tenere = li_mats["liz_border"][:, 1] .!= 0
    ind_aux = ind_Lp[3][lati_tenere, :]
    expansions["mat_map_Lp"][3, 2] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][3], size(ind_Lp[3], 1))
    lati_tenere = li_mats["liz_border"][:, 2] .!= 0
    ind_aux = ind_Lp[3][lati_tenere, :]
    expansions["mat_map_Lp"][3, 3] = sparse(ind_aux[:, 1], ind_aux[:, 2], ones(size(ind_aux, 1), 1), expansions["N_ind"][3], size(ind_Lp[3], 1))
    ...
    ports["surf_s_port_nodes"] = Array{Any}(0)
    ports["surf_e_port_nodes"] = Array{Any}(0)
