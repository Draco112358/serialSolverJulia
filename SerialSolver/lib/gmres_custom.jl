using Profile, FLoops

function gmres_custom(b, restarted, tol, maxit, x , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector)
    # @profile begin
    ...
    r = b - ComputeMatrixVector(x , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector);
    ...
    for outiter = 1:outer 
        ...
        for initer = 1:inner
            ...
            # Apply A to v.
            v = ComputeMatrixVector(v , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector);
            ...
            if (normr <= tolb || stag >= maxstagsteps || moresteps == 1)
                ...
                r = b - ComputeMatrixVector(xm , wk, incidence_selection, FFTCP, FFTCLp, DZ, Yle, expansions, invZ, invP, lu, PLIVector, PVector, PLI2Vector, P2Vector);
                if norm(r) <= tol * n2b
                    x = xm
                    flag = 0
                    iter = [outiter, initer]
